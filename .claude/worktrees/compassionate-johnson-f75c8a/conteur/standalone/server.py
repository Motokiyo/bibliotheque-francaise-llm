        ...
        if et in ("response.output_audio.delta", "response.audio.delta"):
            ...
            if delta:
                try:
                    ...
                    raw_bytes = None
                    chunk_start_offset = audio_bytes_total["n"]
                if dsp_enabled["on"] and active_perso["name"]:
                    profile = profile_for_perso(active_perso["name"], current_annotations)
                    try:
                        raw = raw_bytes if raw_bytes is not None else base64.b64decode(delta)
                        # Mark the start of the current DSP accumulation window.
                        # First raw chunk that enters an empty buffer carries its
                        # own offset; subsequent chunks accumulate and the offset
                        # of the *first* chunk stays put until the next flush.
                        if dsp_buffer_state["start_offset"] is None:
                            dsp_buffer_state["start_offset"] = chunk_start_offset
                        processed = dsp_proc.feed(raw, profile)
                        if processed:
                            emit_offset = dsp_buffer_state["start_offset"]
                            dsp_buffer_state["start_offset"] = None
                            delta = base64.b64encode(processed).decode("ascii")
                        ...
                        logger.warning("DSP relay failed: %s", exc)
                async with send_lock:
                    await ws.send_json({"type": "audio.delta", "data": delta,
                                        "perso": active_perso["name"],
            ...
            new_perso, match_end_pos = detect_perso_with_pos(
                transcript_buffer["text"], current_annotations, all_persos=pool,
            )
            if new_perso and new_perso != active_perso.get("name") and new_perso != active_perso.get("pending"):
                # Convert transcript char position → audio byte offset estimate.
                # The browser holds the AUTHORITATIVE timing: when audio playback
                # reaches this byte_offset, it sends "apply_switch" back to us.
                # No calculated delay — we rely on actual playback timing.
                #
                # Adaptive rate: once we have enough samples, derive the actual
                # chars/sec from Cedar's own output instead of the 17 const.
                # 48000 = bytes/sec for PCM16 24kHz mono.
                ...
                at_byte = int(match_end_pos / rate_used * 48000)
                logger.info(
                    "schedule_switch perso=%s at_audio_byte=%d (transcript pos=%d, rate=%.2f %s)",
                    new_perso, at_byte, match_end_pos, rate_used, rate_source,
                )
                pending_schedule_ts[new_perso] = time.time()
                active_perso["pending"] = new_perso
                async with send_lock:
                    await ws.send_json({
            ...
            if cid and name and conteur:
                try:
                    ...
                    args = {}
                result = _handle_tool_call(name, args, current_oeuvre)
        ...
        while True:
            ...
            if "text" in msg and msg["text"] is not None:
                ...
                if mtype == "start":
                    ...
                    await conteur.push_oeuvre_text(data["scene_text"])
...
def _handle_tool_call(name: str, args: dict, oeuvre: dict | None) -> dict:
    if not oeuvre:
    ...
    if name == "load_full_play":
        return {"text": oeuvre.get("text_complet", "")}
    if name == "list_scenes":
