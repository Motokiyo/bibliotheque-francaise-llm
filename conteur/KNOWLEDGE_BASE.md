...
## §1bis — Cedar V1 vs V2 (vérité durable)

**Cedar V2 ≠ Cedar V1**. OpenAI a rentraîné la voix `cedar` entre `gpt-realtime` (août 2025) et `gpt-realtime-2` (mai 2026) parce que le stack audio sous-jacent du modèle a changé. La voix est encodée dans les audio tokens du modèle de génération unifié — pas un post-filtre désactivable.

**Pas de paramètre `voice_version`**. Si on bascule sur `gpt-realtime-2`, on entend automatiquement Cedar V2. Pas de moyen de forcer Cedar V1 sur le nouveau modèle.

**Conséquence stratégique** pour Reachy Care / Aristote / Douze : tant que la persona vocale ne change pas, on **reste sur `gpt-realtime` V1**. Le jour où OpenAI déprécie V1, on devra basculer (et le résident entendra une voix différente). Solution structurelle long terme = découplage TTS, voir `docs/DECOUPLAGE_TTS_LONGTERME.md`.

## §13 — DSP browser-side via SoundTouch streaming (architecture FINALE 12/05/2026)

### Pourquoi pas le DSP server-side

Le DSP server-side `pyrubberband` était fondamentalement impossible à synchroniser avec ce que le speaker joue :

1. **OpenAI Realtime streame ~5× plus vite que temps réel**. À t_wall = 2.5 sec, le serveur a déjà reçu et envoyé ~13 sec d'audio. Le browser bufferise cet audio dans `playbackTime` qui pointe ~10 sec dans le futur AudioContext.
2. **Le transcript arrive ~500 ms après l'audio correspondant**. Quand le serveur détecte "ŒNONE." dans le transcript, Cedar a déjà parlé le nom dans l'audio EMIS il y a 500 ms — et bufferisé côté browser. Le DSP serveur change le profil pour les chunks SUIVANTS, donc l'audio déjà émis est joué avec l'ancien profil.
3. **Le scheduling `at_byte = match_end_pos / rate * 48000` ne marche pas**. Le browser a un `chunkSchedule` qui mappe byte_offset → AudioContext time. Si `rate` est calculé à partir du transcript (en retard), `at_byte` pointe loin dans le futur AudioContext — le browser fait `setTimeout` de 10-70 sec, le switch arrive trop tard.

### Architecture browser-side adoptée

```
BufferSource (par chunk) → pitchNode (AudioWorkletNode 'streaming-pitch-shifter')
                        → vibratoGain (modulé AM par vibratoLFO × vibratoDepthGain)
                        → masterGain (gain_db → linear, ramp 30ms)
                        → audioCtx.destination
```

- `pitchNode` : `AudioWorkletProcessor` maison ajouté à la fin de `soundtouch-worklet.js` (en plus du processor original buffer-pre-loaded). Réutilise les classes internes `SoundTouch` et `FifoSampleBuffer` de la lib. Push input mono → interleave stéréo → `inputBuffer.putSamples` → `pipe.process()` → `outputBuffer.receiveSamples` → de-interleave → output mono. Latence interne ~50-150 ms (premiers frames silencieux le temps que le buffer SoundTouch se remplisse), steady-state uniforme.
- `vibratoGain` : `GainNode` dont `gain.value` est modulée en amplitude par un LFO `OscillatorNode` × `vibratoDepthGain`. Produit un tremblement vocal (Œnone 2.5 Hz, depth 0.04).
- `masterGain` : `GainNode` pour le `gain_db` du profil (converti en linéaire).
- Tous les ramps via `setTargetAtTime` avec time constant 30 ms pour éviter les clics.

### API d'update

Côté browser, à chaque event `perso.active` reçu :
```js
applyPersoProfile(persoName) → lookup currentAnnotations → set params worklet:
  ...
  vibratoLFO.frequency.setTargetAtTime(p.vibrato_hz, t, 0.03)
  vibratoDepthGain.gain.setTargetAtTime(p.vibrato_depth, t, 0.03)
  masterGain.gain.setTargetAtTime(10**(p.gain_db/20), t, 0.03)
```

### Limites connues

- Latence transcript ~500 ms reste : la détection serveur arrive après l'audio joué. On entend la fin de la réplique précédente et le nom du nouveau perso avec l'ancien profil. Non résolvable sans timing audio→transcript exact d'OpenAI.
- Antennes (Reachy Mini) restent serveur-side via `set_perso_antennas`, donc subissent la même latence transcript.

## §14 — Pipeline audio bout en bout (verite operationnelle)

### Référentiels temporels distincts

| Espace | Échelle | Qui le maintient |
|---|---|---|
| Wall clock (`time.time()`) | secondes UTC | serveur |
| AudioContext `currentTime` | secondes depuis création du context | browser |
| `playbackTime` (browser) | scheduled audio time = somme cumulée des durations | browser |
| `audio_bytes_total` (serveur) | bytes PCM émis cumulés (48000 B/sec) | serveur |
| Transcript `chars_recv_total` | chars de transcript reçus cumulés | serveur |

### Asymétries clés

- `audio_bytes_total` croît à 5× temps réel (streaming OpenAI rapide).
- `audioCtx.currentTime` croît à 1× temps réel.
- `playbackTime` = `currentTime + audio_bufferé` (~10-15 sec en pic).
- Transcript chars croît avec retard ~500 ms par rapport à audio.

### Conséquences pour le timing

- Toute prédiction "à quel moment Cedar dira X" basée sur le transcript est faussée par le lag.
- Toute prédiction "à quel byte sera l'audio à l'instant t" basée sur audio_bytes_total est faussée par le buffering browser.
- Seul timing fiable = lecture côté browser via `audioCtx.currentTime` au moment de la lecture réelle. D'où le pivot DSP browser-side.

## §15 — Découplage TTS long terme (résumé)

Document de référence : `docs/DECOUPLAGE_TTS_LONGTERME.md`.

Recommandation : Option B = OpenAI Realtime en `output_modalities: ["text"]` + ElevenLabs Pro Voice Clone Cedar V1 via LiveKit Agents. Découple la voix d'OpenAI à perpétuité. Latence ajoutée 75-300 ms. Coût ~250 €/mois EHPAD.

**PoC urgent** : capturer 30 min de Cedar V1 dès cette semaine sur contenu varié (dialogue + narration + émotion). Si OpenAI déprécie V1 avant capture, voix perdue à jamais.

**Plan B juridiquement propre** : enregistrer un comédien francophone (300-800 € studio) au timbre proche Cedar, cloner ce comédien. Voix indépendante d'OpenAI, librement utilisable commercialement.
