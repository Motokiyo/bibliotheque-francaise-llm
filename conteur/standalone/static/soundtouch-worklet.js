...
/* ────────────────────────────────────────────────────────────────────────────
 * Streaming SoundTouch processor — added for Cedar Conteur.
 *
 * The upstream SoundTouchWorklet above is built around a pre-loaded
 * AudioBuffer (pull model). Our case is live audio chunks from OpenAI
 * Realtime: push samples in via the audio graph, get pitch-shifted samples
 * out the other side. This class wires the existing SoundTouch pipe in a
 * standard process(inputs, outputs) loop.
 *
 * Input  : mono Float32 (we duplicate to stereo because SoundTouch is stereo-
 *          interleaved internally).
 * Output : mono Float32 (we take the left channel of the stereo output).
 *
 * Pitch / tempo are updated via port.postMessage({ name: 'pitchSemitones',
 * value: 1.0 }) etc., set straight on the internal SoundTouch pipe.
 *
 * The pipe has an inherent latency (~50–150 ms): the first frames out are
 * silent until the internal buffer fills. Steady-state latency is uniform.
 * ──────────────────────────────────────────────────────────────────────── */
registerProcessor('streaming-pitch-shifter', class extends AudioWorkletProcessor {
  constructor() {
    super();
    this._pipe = new SoundTouch();
    this._pipe.sampleRate = sampleRate;
    this._pipe.pitchSemitones = 0;
    this._pipe.tempo = 1.0;
    this._scratchIn = new Float32Array(128 * 2);
    this._scratchOut = new Float32Array(128 * 2);
    this.port.onmessage = (e) => {
      const d = e.data;
      if (!d || typeof d.name !== 'string') return;
      try { this._pipe[d.name] = d.value; } catch (_) {}
    };
  }
  ...
  process(inputs, outputs) {
    ...
    if (this._scratchIn.length < len * 2) {
      this._scratchIn = new Float32Array(len * 2);
      this._scratchOut = new Float32Array(len * 2);
    }
    for (let i = 0; i < len; i++) {
      this._scratchIn[i * 2] = inL[i];
      this._scratchIn[i * 2 + 1] = inR[i];
    }
    this._pipe.inputBuffer.putSamples(this._scratchIn, 0, len);
    this._pipe.process();
    ...
    if (toRead > 0) {
      this._pipe.outputBuffer.receiveSamples(this._scratchOut, toRead);
      for (let j = 0; j < toRead; j++) {
        outL[j] = this._scratchOut[j * 2];
        if (outR) outR[j] = this._scratchOut[j * 2 + 1];
      }
      for (let k = toRead; k < len; k++) {
        outL[k] = 0;
        if (outR) outR[k] = 0;
      }
    } else {
